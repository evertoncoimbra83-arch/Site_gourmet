...
* **Entregas**:
  ...
  * Captura de diffs *before/after* para alterações administrativas em orders, settings, fidelidade, cupons, logística e métodos de pagamento.
...
2. **Sprint OAuth P1 — Infraestrutura de Integração Social**:
   * Criação da tabela `user_oauth_accounts` e modelagem Drizzle.
   * Lógica de conexão segura com a API do Google OAuth2.0 (state, nonce e verificação PKCE).
3. **Sprint OAuth P2 — Login/Cadastro Google**:
...
4. **Sprint OAuth P3 — Conexões no Painel Logado**:
