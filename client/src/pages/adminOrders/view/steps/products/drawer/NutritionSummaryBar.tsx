// products/drawer/NutritionSummaryBar
...
interface NutritionSummaryBarProps {
  data: NutritionMacros | null | undefined;
  selectedAccompaniments?: NutritionMacros[];
  onShowFull: () => void;
}
...
export const NutritionSummaryBar = ({ 
  ...
  const stats = useMemo(() => {
    // ✅ Tipagem do parâmetro da função interna para evitar 'any'
    ...
    if (selectedAccompaniments.length > 0) {
      selectedAccompaniments.forEach(acc => {
        totalKcal += p(acc.energyKcal || acc.kcal);
        totalCarbs += p(acc.carbs);
        totalPro += p(acc.proteins || acc.protein);
        totalFat += p(acc.fatTotal || acc.fats);
      });
    }
  ...
  return (
    <div className="bg-white border-y border-slate-100 p-6 space-y-4 animate-in fade-in slide-in-from-bottom-2 text-left">
        ...
        <div className="flex justify-between px-1">
          {items.map((item) => (
            <div key={item.label} className="flex flex-col items-start">
              ...
              <span className="text-[11px] font-black text-slate-800">{item.val.toFixed(1)}<span className="text-[8px] ml-0.5 text-slate-500">g</span></span>
            </div>
          ))}
        </div>
      </div>
