...
export function CategoryIcon({ iconKey, color, size = 18 }: CategoryIconProps) {
  // 1. Robustez de Casing e Tipagem Segura
  const formattedKey = iconKey 
    ? iconKey.charAt(0).toUpperCase() + iconKey.slice(1).toLowerCase() 
    : "Tag";

  // ✅ CORREÇÃO: Substituído 'any' por mapeamento de tipos do LucideIcons
  const IconComponent = (LucideIcons as unknown as Record<string, LucideIcons.LucideIcon>)[formattedKey] || LucideIcons.Tag;
