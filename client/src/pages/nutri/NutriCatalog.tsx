...
interface DishSize {
  ...
  mainDishWeight?: string | number | null;
  energyKcal?: number | string | null;
  proteins?: number | string | null;
  carbs?: number | string | null;
  fatTotal?: number | string | null;
}
...
interface CatalogDish {
  ...
  // Campos extras comuns em catálogos de nutrição
  energyKcal?: number | string | null;
}
    ...
    <div className="p-6 space-y-8 text-left bg-slate-50/50 min-h-screen">
      ...
      <div className="flex justify-between items-end px-2">
        ...
        <div className="flex gap-3">
          ...
          <Button className="bg-slate-900 hover:bg-slate-800 text-white font-black uppercase text-[10px] tracking-widest h-14 px-8 rounded-2xl shadow-xl shadow-slate-200 transition-all active:scale-95 flex gap-3 border-none">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {dishes?.filter(d => d.name.toLowerCase().includes(searchTerm.toLowerCase())).map((dish) => {
          ...
          // Fallback seguro para exibição de dados
          const firstSize = d.availableSizes?.[0] || {};
          const displayWeight = firstSize.weight || firstSize.mainDishWeight || "---";
          const displayKcal = firstSize.energyKcal || d.energyKcal || "---";
          ...
          return (
            <div key={d.id} className="group p-6 bg-white border border-slate-100 rounded-4xl shadow-sm hover:shadow-xl hover:shadow-emerald-500/5 transition-all duration-300 flex flex-col gap-6 relative overflow-hidden text-left">
              ...
              <div className="flex gap-4 items-center">
                ...
                <div className="flex flex-col justify-center gap-1 text-left min-w-0">
                  <h4 className="font-black text-lg text-slate-800 uppercase leading-tight tracking-tighter truncate">
                    {d.name}
                  </h4>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest pl-2">
                      {d.availableSizes?.length || 0} Tamanhos
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-slate-50 p-4 rounded-2xl flex items-center gap-3 border border-slate-100/50">
                  ...
                  <div className="flex flex-col text-left">
                    <span className="text-[10px] font-black text-slate-800 uppercase leading-none">{displayKcal} Kcal</span>
                    <span className="text-[8px] font-bold text-slate-400 uppercase">Energia</span>
                  </div>
                </div>
                <div className="bg-slate-50 p-4 rounded-2xl flex items-center gap-3 border border-slate-100/50">
