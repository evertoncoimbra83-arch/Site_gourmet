...
/**
 * ✅ HOOK: USE DISH NUTRITION (PADRONIZADO 100g)
...
export function useDishNutrition(
  ...
  return useMemo(() => {
    // 1. Estado Inicial (13 Campos do Schema)
    ...
    const getVal = (source: Record<string, unknown> | null | undefined, ...keys: string[]): number => {
      ...
      // Procura na raiz e dentro de objetos de info (Casting seguro para satisfazer o linter)
      const roots = [
        source, 
    ...
    const finalMainWeight = mainWeight > 0 ? mainWeight : 200;
    
    // ✅ PADRÃO 100g: Assume que o banco de dados guarda valores por 100g
    const dishFactor = finalMainWeight / 100;
    ...
    totals.yieldWeight += finalMainWeight;

    // =========================================================
    // B. CÁLCULO DOS ACOMPANHAMENTOS
    // =========================================================
    selectedAccs.forEach((opt) => {
