// E:\IA\projects\Site_React\client\src\pages\products\drawer\NutritionSummaryBar.tsx
...
interface NutritionSummaryBarProps {
  ...
  onShowFull?: () => void; // ✅ Tornou-se opcional para o modo Slim
  variant?: "default" | "slim"; // ✅ Nova prop para controlar o visual
}
...
export const NutritionSummaryBar = ({ 
  ...
  const stats = useMemo(() => {
    const p = (v: string | number | undefined | null) => {
      ...
      return safeNumber(String(v || 0).replace(",", "."));
    };
  ...
  // ✅ RENDERIZAÇÃO MODO SLIM (Para a Tab de Pedidos)
  if (variant === "slim") {
    return (
      <div className="space-y-2 animate-in fade-in duration-500">
        <div className="flex justify-between items-center mb-1">
          <span className="text-[8px] font-black uppercase text-slate-400 tracking-widest flex items-center gap-1">
            <PieChart size={10} className="text-emerald-500" /> DNA Nutricional
          </span>
          <span className="text-[11px] font-black text-slate-900 leading-none">
            {stats.kcal} <span className="text-[8px] text-emerald-600 tracking-tighter">KCAL</span>
          </span>
        </div>

        {/* Barra de progresso ultra fina */}
        <div className="h-1.5 w-full flex rounded-full overflow-hidden bg-slate-100 shadow-inner">
          {items.map((item) => (
            <div 
              key={item.label}
        ...
        {/* Legenda compacta em uma linha */}
        <div className="flex justify-between gap-2">
          {items.map((item) => (
            <div key={item.label} className="flex items-center gap-1">
              <div className={cn("w-1 h-1 rounded-full", item.color)} />
              <span className="text-[8px] font-bold text-slate-500 uppercase">
                {item.val.toFixed(0)}g <span className="opacity-50 font-medium">{item.label[0]}</span>
              </span>
            </div>
          ))}
        </div>
  ...
  // ✅ RENDERIZAÇÃO MODO DEFAULT (Para o Drawer de Produtos)
  return (
          ...
          {items.map((item) => (
            <div key={item.label} className="flex flex-col items-start">
              ...
              <span className="text-[11px] font-black text-slate-800">
                {item.val.toFixed(1)}<span className="text-[8px] ml-0.5 text-slate-500">g</span>
              </span>
            </div>
          ))}
        </div>
