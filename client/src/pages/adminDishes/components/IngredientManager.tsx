...
const EMPTY_FORM_DATA: IngredientFormData = {
  name: "",
  yieldFactor: "1.00",
  energyKcal: "0",
  energyKj: "0",
...
export function IngredientManager() {
  ...
  const [formData, setFormData] = useState<IngredientFormData>({ ...EMPTY_FORM_DATA });
  ...
  const { data: ingredients, isLoading } = trpc.admin.ingredients.list.useQuery({
  ...
  const paginatedItems = useMemo(() => {
    ...
    return (ingredients as unknown as LocalIngredient[]).slice(
      start,
      start + itemsPerPage,
    );
  }, [ingredients, page]);
  ...
  const handleImportExternal = async (queryOverride?: string) => {
    ...
    toast.info(
      isFatSecret ? "Processando link do FatSecret..." : "Consultando base técnica.",
      {
    ...
    try {
      const results = await utils.admin.ingredients.searchExternal.fetch({
        name: query,
      });
      ...
      toast.error("Falha na conexão.", {
        description: "Verifique o link ou sua internet.",
      });
    } finally {
  ...
  const selectProduct = (prod: ExternalProduct) => {
    const brandSuffix = prod.brand ? ` (${prod.brand})` : "";

    setFormData((prev) => ({
      ...prev,
      name: `${prod.name}${brandSuffix}`,
    ...
    setExternalResults([]);
    setExternalQuery("");
    toast.success("Tabela nutricional carregada.");
  };
  ...
  const handleOpenDetails = (ing: LocalIngredient) => {
    ...
    setFormData({
      name: ing.name,
      yieldFactor: String(ing.yieldFactor || "1.00"),
      energyKcal: String(ing.energyKcal || "0"),
      energyKj: String(ing.energyKj || "0"),
      proteins: String(ing.proteins || "0"),
      carbs: String(ing.carbs || "0"),
      addedSugars: String(ing.addedSugars || "0"),
      fatTotal: String(ing.fatTotal || "0"),
      fatSaturated: String(ing.fatSaturated || "0"),
      fatTrans: String(ing.fatTrans || "0"),
      fiber: String(ing.fiber || "0"),
      sodium: String(ing.sodium || "0"),
    });
  ...
  const handleDeleteConfirmed = async () => {
    ...
    await deleteMutation.mutateAsync({ id: selectedIngId });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center">
        ...
        <Button
          ...
          className="h-14 px-8 rounded-2xl bg-slate-950 hover:bg-emerald-600 text-white font-black uppercase text-[10px] tracking-widest transition-all w-full md:w-auto shadow-lg active:scale-95"
        >
            ...
            >
              <div className="flex items-center gap-3 w-full">
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="font-black text-slate-800 uppercase text-[10px] leading-tight italic line-clamp-2">
                    {ing.name}
                  </h3>
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter mt-1">
                    {ing.energyKcal} kcal / 100g
                  </p>
                </div>
              </div>
              ...
              <Info
                size={14}
                className="text-slate-200 group-hover:text-emerald-500 ml-2"
              />
            </div>
          ))}
        </div>
        ...
        <DialogContent className="sm:max-w-4xl rounded-4xl p-8 border-none max-h-[95vh] flex flex-col bg-white overflow-hidden shadow-2xl">
          <DialogHeader className="mb-4 text-left">
            <DialogTitle className="text-3xl font-black uppercase italic tracking-tighter">
              {selectedIngId ? "Detalhes do" : "Novo"}{" "}
              <span className="text-emerald-500">Insumo</span>
            </DialogTitle>
            <DialogDescription className="sr-only">
              Formulário técnico para gestão nutricional.
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="p-4 bg-emerald-50/50 rounded-[1.5rem] border border-emerald-100 flex gap-3 items-end">
                <div className="flex-1 space-y-2">
                  <Label className="text-[9px] font-black uppercase text-emerald-600 ml-2 flex items-center gap-1">
                    <Globe size={12} />
                    Busca Inteligente (Nome, Link FatSecret ou Barcode)
                  </Label>
              ...
              {externalResults.length > 0 && (
                <div className="space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
                  <div className="flex justify-between items-center px-2">
                    <span className="text-[9px] font-black uppercase text-slate-400 italic tracking-wider">
                      Sugestões ({externalResults.length})
                    </span>

                    <button
                      type="button"
                      onClick={() => setExternalResults([])}
                      className="text-[9px] font-black uppercase text-red-400 hover:text-red-500 flex items-center gap-1"
                    >
                      <X size={10} />
                      Limpar
                    </button>
          ...
          {/* CONFIGURAÇÃO EXPLICITA DAS PROPS SEM ANY: Mapeamento direto inferido com base nos tipos nativos do IngredientForm */}
          <div className="flex-1 overflow-y-auto custom-scrollbar pr-2">
            <IngredientForm
              formData={formData}
              setFormData={setFormData as unknown as React.Dispatch<React.SetStateAction<Parameters<typeof IngredientForm>[0]["formData"]>>}
              handleKcalChange={(kcal) => {
                const val = safeNumber(kcal);
                setFormData((prev) => ({
                  ...prev,
                  energyKcal: kcal,
                  energyKj: (val * 4.184).toFixed(2),
                }));
              }}
            />
          </div>
          ...
          <DialogFooter className="pt-8 border-t border-slate-50 gap-4 mt-auto">
            ...
            {/* CONFIGURAÇÃO DO PAYLOAD SEM ANY: Reaproveitamento estrito de assinaturas do procedimento do tRPC */}
            <Button
              onClick={() => {
                const payload = {
                  ...formData,
                  id: selectedIngId || undefined,
                };
      ...
      <ConfirmDialog
        ...
        description="Este insumo será removido permanentemente da base técnica."
